...
GunnerEnemy::GunnerEnemy()
{
	...
	static SDL_Texture* tex_enemy = texture_pool.find(ResID::Tex_Gunner)->second;
	static SDL_Texture* tex_enemy_sketch = texture_pool.find(ResID::Tex_GunnerSketch)->second;
	static ConfigManager::EnemyTemplate& enemy_template = ConfigManager::instance()->gunner_template;

	anim_idle_left.set_loop(true);
	anim_idle_left.set_interval(0.3);
	anim_idle_left.set_frame_data(tex_enemy, 2, 6, { 0,1 });
	anim_idle_right.set_loop(true);
	anim_idle_right.set_interval(0.3);
	anim_idle_right.set_frame_data(tex_enemy, 2, 6, { 6,7 });

	anim_move_left.set_loop(true);
	anim_move_left.set_interval(0.1);
	anim_move_left.set_frame_data(tex_enemy, 2, 6, { 2,3 });
	anim_move_right.set_loop(true);
	anim_move_right.set_interval(0.1);
	anim_move_right.set_frame_data(tex_enemy, 2, 6, { 8,9 });

	anim_ready_left.set_loop(false);
	anim_ready_left.set_interval(0.2);
	anim_ready_left.set_frame_data(tex_enemy, 2, 6, { 0,1 });
	anim_ready_right.set_loop(false);
	anim_ready_right.set_interval(0.2);
	anim_ready_right.set_frame_data(tex_enemy, 2, 6, { 6,7 });

	anim_attack_left.set_loop(false);
	anim_attack_left.set_interval(0.1);
	anim_attack_left.set_frame_data(tex_enemy, 2, 6, { 4,5 });
	anim_attack_right.set_loop(false);
	anim_attack_right.set_interval(0.1);
	anim_attack_right.set_frame_data(tex_enemy, 2, 6, { 10,11 });

	anim_idle_left_sketch.set_loop(true);
	anim_idle_left_sketch.set_interval(0.3);
	anim_idle_left_sketch.set_frame_data(tex_enemy_sketch, 2, 6, { 0,1 });
	anim_idle_right_sketch.set_loop(true);
	anim_idle_right_sketch.set_interval(0.3);
	anim_idle_right_sketch.set_frame_data(tex_enemy_sketch, 2, 6, { 6,7 });

	anim_move_left_sketch.set_loop(true);
	anim_move_left_sketch.set_interval(0.1);
	anim_move_left_sketch.set_frame_data(tex_enemy_sketch, 2, 6, { 2,3 });
	anim_move_right_sketch.set_loop(true);
	anim_move_right_sketch.set_interval(0.1);
	anim_move_right_sketch.set_frame_data(tex_enemy_sketch, 2, 6, { 8,9 });

	anim_ready_left_sketch.set_loop(false);
	anim_ready_left_sketch.set_interval(0.2);
	anim_ready_left_sketch.set_frame_data(tex_enemy_sketch, 2, 6, { 0,1 });
	anim_ready_right_sketch.set_loop(false);
	anim_ready_right_sketch.set_interval(0.2);
	anim_ready_right_sketch.set_frame_data(tex_enemy_sketch, 2, 6, { 6,7 });

	anim_attack_left_sketch.set_loop(false);
	anim_attack_left_sketch.set_interval(0.1);
	anim_attack_left_sketch.set_frame_data(tex_enemy_sketch, 2, 6, { 4,5 });
	anim_attack_right_sketch.set_loop(false);
	anim_attack_right_sketch.set_interval(0.1);
	anim_attack_right_sketch.set_frame_data(tex_enemy_sketch, 2, 6, { 10,11 });
...
void GunnerEnemy::attack(double delta)
{
	...
	{
		if (!have_caused_damage)
		{
			have_caused_damage = true;
			BulletManager::instance()->fire_bullet(Enemybullet,velocity, 200, position, position, view_radius, damage, {});
		}
	}
}